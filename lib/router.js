...
function outputFile(filePath, content) {
	fs.writeFile(filePath, content, function(err) {
		if (err) throw err;
		console.log(`${filePath} is saved!`); //文件被保存
	});
}

// 首页模版, 渲染template/index.html
router.get('/', function*() {
	let [config, rootFn] = yield [loadConfig(), util.loadTemplate('index')]
	// config是app.json merge 默认值
	...
	this.type = 'html'
		//yield next
	outputFile(path.resolve('./dist/index.html'), this.body);
})

// 渲染template/service.html
...
router.get(tmpDir + '/(.*)', function*() {
	yield send(this, this.request.path, {
		root: root
	})
})
